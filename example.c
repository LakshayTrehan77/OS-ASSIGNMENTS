...
void sigsegv_handler(int signo) {
    if (signo == SIGSEGV) {
        printf("Segmentation fault occurred at line %d.\n", __LINE__);
        // Add your custom message or actions here
        exit(EXIT_FAILURE); // Or you can choose to handle it gracefully and continue.
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    printf("Virtual Address (ptr[0][1]): %lu\n", &phy_ptr[0]);
    printf("Virtual Address (ptr[0][0]): %lu\n", &phy_ptr2[1]);
    ...
    */
    ...
    ptr[11] = (int*)mems_malloc(sizeof(int)*4);
    ptr[10] = (int*)mems_malloc(sizeof(int)*1000);
    ptr[12] = (int*)mems_malloc(sizeof(int)*500);
    mems_free(ptr[12]);
